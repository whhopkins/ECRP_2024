...
% * try and motivate more use cases for your general framework in SALT - what other physics objects could benefit? anything additional at ANL? outside of ANL? and what about physics analyses. some examples would be helpful. we definitely have analyses that struggle with systematic uncertainties. 
...
% The objective of the proposed work is to enhance the discovery potential of the HL-LHC by both improving the statistical power of measurements and by reducing the effect of systematic uncertainties on precision measurements. This is enabled by the incorporation of domain-adaptation techniques into an ML framework. Domain adaptation will first be applied to the ATLAS photon ID and energy evaluation which will in turn be used in the upcoming \hyy\ differential cross-section measurements. Additionally, the proposed domain-adaptation techniques will be used to improve the signal-vs-background discriminator in the \hyy\ differential cross-section measurement. The ML framework resulting from the proposed work will provide streamlined access to domain-adaptation techniques that automatically mitigate the effects of systematic uncertainties and simplify calibration tasks (e.g., adjusting simulations to better match recorded data), for a wide range of applications within HEP.
% \begin{itemize}
% \item Improve the precision of future HEP measurements
%   \begin{itemize}
%   \item By incorporating resistance to parameter changes (e.g., resolution in the assessment of systematic uncertainties), data-simulation discrepancies, and changing detector conditions into ML models through the use of domain-adaptation techniques
%   \end{itemize}
% \item Create a framework that inherently integrates domain-adaptation techniques during the development of ML models
% \item Use ML framework with domain adaptation to improve Run~3 \hyy\ differential cross-section measurement
% \end{itemize}
...
% mention how challenging some of these methods are (adversarial)
...
% A specific example might be good
...
%     & {\bf PI} & {\bf Postdoc 1} & {\bf Postdoc 2} & {\bf Student} \\  \hline\hline 
%     Adversarial & used & complex correlations & high \\ \hline
%     Distance Correlation & used & non-linear correlations & low \\ \hline
%     Feature Transfer & not used & features for multiple tasks  & medium \\ \hline\hline                                                                                                                                       
%   \end{tabular}
